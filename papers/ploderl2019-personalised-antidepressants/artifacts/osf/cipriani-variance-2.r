...
vr.var = 1/(2*(dat.all$placebo.n-1)) + 1/(2*(dat.all$all.n-1)) # Equation 10 in Nakagawa 2015
...
rdat <- escalc(measure = analysis, 
               ...
               data = dat.all)
m.all  <- rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = T) # same parameters as in Winkelbauer et al. 2019
m.all  <- rma(yi = yi, vi = vi, data = rdat, method = "DL", slab = dat.all$Study, weighted = T, test="knha") 
...
# rdat$yi-vr     # Check if calculation in escalc is according to Nakagawa, results must be zero
# rdat$vi-vr.var # Check if calculation in escalc is according to Nakagawa, results must be zero
# Check convergence , www.metafor-project.org/doku.php/tips:convergence_problems_rma
tmp = capture.output(rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = T))
check1=rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = T,verbose=T) # check how many interations are necessary
check2=rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = T,verbose=T,control=list(tau2.init=0.50))
check3=rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = T,verbose=T,control=list(tau2.min=-min(rdat$vi))) # Check in "Conducting Meta-Analysis in R with the metafor Package" by Wolfgang Viechtbauer
...
profile(check3,xlim=c(-0.01,0.05))
...
vr=log(dat.ssri$ssri.sd/dat.ssri$placebo.sd) + 1/(2*(dat.ssri$ssri.n-1)) - 1/(2*(dat.ssri$placebo.n-1)) # Equation 9 in Nakagawa 2015
vr.var = 1/(2*(dat.ssri$placebo.n-1)) + 1/(2*(dat.ssri$ssri.n-1)) # Equation 10 in Nakagawa 2015
...
vr=log(dat.snri$snri.sd/dat.snri$placebo.sd) + 1/(2*(dat.snri$snri.n-1)) - 1/(2*(dat.snri$placebo.n-1)) # Equation 9 in Nakagawa 2015
vr.var = 1/(2*(dat.snri$placebo.n-1)) + 1/(2*(dat.snri$snri.n-1)) # Equation 10 in Nakagawa 2015
...
#m.snri  <- rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.snri$Study, weighted = TRUE,tau2=0.001)
summary(m.snri)
coef(summary(m.snri)) 
...
vr=log(dat.atyp$atyp.sd/dat.atyp$placebo.sd) + 1/(2*(dat.atyp$atyp.n-1)) - 1/(2*(dat.atyp$placebo.n-1)) # Equation 9 in Nakagawa 2015
vr.var = 1/(2*(dat.atyp$placebo.n-1)) + 1/(2*(dat.atyp$atyp.n-1)) # Equation 10 in Nakagawa 2015
...
vr=log(dat.tri$tri.sd/dat.tri$placebo.sd) + 1/(2*(dat.tri$tri.n-1)) - 1/(2*(dat.tri$placebo.n-1)) # Equation 9 in Nakagawa 2015
vr.var = 1/(2*(dat.tri$placebo.n-1)) + 1/(2*(dat.tri$tri.n-1)) # Equation 10 in Nakagawa 2015
...
# Drug drop-out 
# Note: this is not significant anymore after the inclusion of one additional study (see revision of manuscript)
...
# Check if efficacy differs for instruments (MADRS is best one!)
dat.all = subset(data2, is.na(all.sd)==F & is.na(all.m)==F & is.na(all.n)==F & outcome==analysis.out)
...
# Meta-regressions, different moderator variables
m.all  <- rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = TRUE)
m.MADRS  <- rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = TRUE,subset=instrument=="MADRS")
m.reg  <- rma(yi = yi, vi = vi, data = rdat, method = "REML", slab = dat.all$Study, weighted = TRUE, mods= ~ instrument)

summary(m.all); plot(m.all)
summary(m.MADRS); plot(m.MADRS)
summary(m.reg); plot(m.reg)
