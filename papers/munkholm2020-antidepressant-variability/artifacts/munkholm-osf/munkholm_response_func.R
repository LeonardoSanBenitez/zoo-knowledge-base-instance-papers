...
simulate_rct <- function(n, id, mu_x, sd_x, delta, mu_y_tx, sd_y_tx, mu_y_ct, sd_y_ct, cutoff) { 
  #
  # simulate rct data frame
  #
  x_tx     <- round(rnorm(n, mu_x, sd_x))
  y_tx     <- round(rnorm(n, mu_y_tx, sd_y_tx))
  x_ct     <- round(rnorm(n, mu_x, sd_x))
  y_ct     <- round(rnorm(n, mu_y_ct, sd_y_ct)) 
  x1       <- c(x_ct[1:(n/2)], x_tx[1:(n/2)]) # 1. RCT
  y1       <- c(y_ct[1:(n/2)], y_tx[1:(n/2)]) # 1. RCT
  x2       <- c(x_tx[(n/2+1):n], x_ct[(n/2+1):n]) # 1. crossover trial
  y2       <- c(y_tx[(n/2+1):n], y_ct[(n/2+1):n]) # 1. crossover trial
  
  #x_tx <- round(rnorm(n/2, y_mean + raw_mu, y_sd))
  #x_ct <- round(rnorm(n/2, y_mean + delta_mu + raw_mu, y_sd))
  
  # data frame
  dat <- data.frame(id = id, x1 = x1, y1 = y1,
    ...
    mutate(id = sample(id), # shuffle ids
           ...
           cs1 = y1-x1,
           cs2 = y2-x2,
           d = cs1-cs2, #it follows from the variables that this equals -delta.
           resp1 = ifelse(cs1 < -cutoff, 1, 0),
           resp2 = ifelse(cs2 < -cutoff, 1, 0))
  
  # correct cs2
  #dat$cs2 <- dat$cs2 * c(rep(-1, n/2), rep(1, n/2)) #I think this creates a faulty plot of the change scores. Unnecessary when x2 and y2 are defined according to tx group. 
  #dat$cs1 <- dat$cs1 * c(rep(-1, n/2), rep(1, n/2)) #I think this creates a faulty plot of the change scores
  ...
  dat <- dat %>%
    select(id, x1, y1, cs1, tx1, everything())
...
gg_forest <- function(fdd, 
  ...
  fp1 <- fdd %>% 
    ggplot(aes(x = fct_reorder(study2, rank), y = es)) +
  ...
  fp2 <- fp1 + 
    ...
    annotate("text", y = ylim[2], x = nrow(fdd) + 1.5, 
             label = paste0(cilab), hjust = 1.1, fontface = "bold") +
    annotate("text", y = ylim[2], x = 1:nrow(fdd), hjust = 1.1,
             label = paste0(
               ...
               "]"), size = 2.5) +
    annotate("text", y = ylim[2], x = nrow(fdd) + 1.5, 
             label = paste0(i2lab), hjust = 0, nudge_x = 0.2,  fontface = "bold") +
    annotate("text", y = ylim[2], x = 1:nrow(fdd), hjust = 0, nudge_x = 0.5,
             label = round(fdd$`I^2 (%)`, 1), size = 2.5) +
    ...
    annotate("text", y = 0.5, x = nrow(fdd) + 1.5,
             label = "N", hjust = 1, fontface = "bold") +
    annotate("text", y = 0.5, x = 1:nrow(fdd), label = paste0(fdd$n), 
             hjust = 1, size = 2.5) 
